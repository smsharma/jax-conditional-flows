...
class MaskedDense(nn.Dense):
  ...
  def __call__(self, inputs: Array, context=None) -> Array:
    ...
    if self.use_context and context is not None:
        assert (inputs.shape[0] == context.shape[0]), "Inputs and context must have the same batch size"
...
class MaskedAutoregressiveBijector(nn.Module):
    bijector_fn: Any
    n_params: Any = 2
    hidden_dims: Any = jnp.array([8, 8])
    activation: str = 'tanh'
        
    @compact
    def __call__(self, y: Array) -> distrax.Bijector:
